    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                ...
                data={
                    "from": f"Omer Shamiah <postmaster@{config.MAILGUN_DOMAIN}>",
                    "to": [to],
                    "subject": subject,
                    "text": body,
                },
...
async def _generate_cute_creature_api(prompt: str):
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            raise APIResponseError(
                f"Api with request failed with status code {err.response.status_code}"
            ) from err
...
async def generate_and_add_to_post(
    ...
    await send_simple_email(
        email,
        "Image geeration completed",
        (
            f"Hi {email}! Your image has been generated and added to your post."
            f" Please clich on the following link to view it: {post_url}"
        ),
    )
