...
@pytest.mark.anyio
async def test_send_simple_emailapi_error(mock_httpx_client):
...
async def test_generate_cute_creature_api_success(mock_httpx_client):
    json_data = {"output_url": "https://example.com/image.png"}
...
async def test_generate_cute_creature_api_server_error(mock_httpx_client):
    ...
    with pytest.raises(
        APIResponseError, match="Api with request failed with status code 500"
    ):
...
@pytest.mark.anyio
async def test_generate_add_to_post_success(
